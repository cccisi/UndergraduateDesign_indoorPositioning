...
//默认参数
	...
	public static final int TRACK_SIZE = 5;//轨迹记录5个点
	...
	@Override
	protected void onDraw(Canvas canvas) {//定位点连线
		super.onDraw(canvas);
		...
		if (showGrid) {
			...
			if (drawOffsetX >= scaledGridWidth)
				drawOffsetX = 0;
			int beginUnitX = gridOffsetX > 0 ? (int)Math.ceil(gridOffsetX / scaledGridWidth) : 0;
			...
			if (drawOffsetY >= scaledGridWidth)
				drawOffsetY = 0;
			int beginUnitY = gridOffsetY > 0 ? (int)Math.ceil(gridOffsetY / scaledGridWidth) : 0;;
